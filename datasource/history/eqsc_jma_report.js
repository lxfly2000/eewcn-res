...
function history_url(){
    if(lastReturnCode<0){//需要重新获取AccessToken
        return "https://equake.top/createAccessToken#eqsc_jma_report";
    }else{//正常获取预警数据
        return "https://equake.top/jma_report.json";
    }
}
...
function history_onsuccess(str_response){
    if(str_response.substr(0,1)==="-"){//错误码
        ...
        var shuju_array=[{
            id:original.Head.EventID,
            O_TIME:original.Body.Earthquake.OriginTime,
            EPI_LAT:original.Body.Earthquake.Hypocenter.Latitude,
            EPI_LON:original.Body.Earthquake.Hypocenter.Longitude,
            EPI_DEPTH:parseFloat(original.Body.Earthquake.Hypocenter.Depth),
            AUTO_FLAG:parse_auto_flag(original.Head.Title),
            EQ_TYPE:"M",
            M:original.Body.Earthquake.Magnitude,
            LOCATION_C:"<b>["+shindo_str(original.Body.Intensity.Observation.MaxInt)+"]</b> "+original.Body.Earthquake.Hypocenter.Name
        }];
...
function is_history_data(url){return url==="https://equake.top/createAccessToken#eqsc_jma_report"||url==="https://equake.top/jma_report.json";}
